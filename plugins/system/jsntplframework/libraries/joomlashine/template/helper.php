...
class JSNTplTemplateHelper
{
		...
		foreach (array('promoColumns', 'mainColumns', 'contentColumns', 'userColumns') AS $row)
		{
			...
			if ($visible < count($params[$row]))
			{
				...
				if (count($columns) == 3 AND $visible == 2)
				{
					...
					{
						$params[$row][$ordering[1]] = preg_replace('/span\d+/i', 'span6', $params[$row][$ordering[1]]);
						$params[$row][$ordering[2]] = preg_replace('/span\d+/i', 'span6', $params[$row][$ordering[2]]);
					}
				}
			}
	...
	private function _prepareHead ()
	{
		...
		! class_exists('JHtmlBootstrap') OR JHtmlBootstrap::loadCss(true, $this->_document->direction);
		...
		if ($this->_loadTemplateJS == true)
		{
			...
			if (is_array($this->_document->templateWidth) AND $this->_document->templateWidth['type'] == 'responsive')
			{
				$enabledLayout = json_encode($this->_document->templateWidth[$this->_document->templateWidth['type']]);
			}
			...
			$this->_document->addScriptDeclaration('
				JSNTemplate.initTemplate({
					...
					enableMobile			: ' . ((isset($this->_document->mobileView) AND $this->_document->mobileView) ? 1 : 0) . ',
