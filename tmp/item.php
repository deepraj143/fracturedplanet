	  ...
	  <ul class="itemCommentsList">
	    ...
	    <li class="<?php echo ($key%2) ? "odd" : "even"; echo (!$this->item->created_by_alias && $comment->userID==$this->item->created_by) ? " authorResponse" : ""; echo($comment->published) ? '':' unpublishedComment'; ?>">
	    	...
	    	<span class="commentLink">
				...
				<?php // K2 - Jomsocial Integration		
				if ($pluginParams->get( 'k2_comm_avatar' )) { 
				$jsuser =& CFactory::getUser($comment->userID); 		 
				?>
				<img src="<?php echo $jsuser->getThumbAvatar(); ?>" alt="<?php echo JFilterOutput::cleanText($comment->userName); ?>" width="<?php echo $this->item->params->get('commenterImgWidth'); ?>" />
				<?php } 
				if (!$pluginParams->get( 'k2_comm_avatar' )) { ?>
		    ...
		    <span class="commentAuthorName">
			    ...
			    <?php // K2 - Jomsocial Integration				    		
					if ($pluginParams->get( 'k2_comm_enable' )) {
					if ($comment->userID!=0) { ?>
	        <a href="<?php echo CRoute::_('index.php?option=com_community&view=profile&userid='.$comment->userID); ?>" title="<?php echo JFilterOutput::cleanText($comment->userName); ?>" target="_blank" rel="nofollow">
			    	<?php echo $comment->userName; ?>
			    </a>
					<?php }
					if ($comment->userID==0) {
					$query = " SELECT id, commentURL "
				 	." FROM #__k2_comments "
				  ." WHERE id=".$comment->id;		 
				  $db =& JFactory::getDBO();
				  $db->setQuery( $query );	
				  $row = $db->loadObject();			 
				  $user_web = $row->commentURL;
					?>
					<a href="<?php echo $user_web; ?>" title="<?php echo JFilterOutput::cleanText($comment->userName); ?>" target="_blank" rel="nofollow">
			    	<?php echo $comment->userName; ?>
			    </a>
					<?php }
	        }
