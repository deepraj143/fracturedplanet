...
class JSNUniformHelper
{
				...
				if (!empty($jsonFile))
				{
					...
					else
					{
						...
						else
						{
							if ($action == "list")
							{
								$contentField = isset($jsonFile->name) ? $jsonFile->name : "N/A";
							}
							else
							{
								$contentField = isset($jsonFile->name) ? "<a href=\"{$link}\">{$jsonFile->name}</a>" : "N/A";
	 ...
	 * @return string
	 */
	public static function generateHTMLPages($formId, $formName, $formType = "", $topContent = "", $bottomContent = "")
	{
		$baseUrl = JURI::base(true);
		$html = "";
		// Load language
		...
		if ($items)
		{
			...
			if (!$formType)
			{
				...
				$html .= "<input type='hidden' name='language' value='" . json_encode(JSNUniformHelper::getTranslated($arrayTranslated)) . "'/>";
			...
			foreach ($formPages as $i => $contentForm)
			{
				...
				{
					$htmlForm .= JSNFormGenerateHelper::generate($formTemplate->dataField, $formTemplate->dataFormLayout, $dataSumbission, $pageContainer);
				}
