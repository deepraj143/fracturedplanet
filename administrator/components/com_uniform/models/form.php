				...
				if (isset($post['name_page']))
				{
						...
						foreach ($post['name_page'] as $value => $text)
						{
								...
								if ($dataFormId == 0)
								{
										...
										$pageContainer = json_decode($session->get('form_container_page_' . $value, '', 'form-design-'));
								}
								else
								{
										$formFields = json_decode($session->get('form_page_' . $value, '', 'form-design-' . $dataFormId));
										$pageContainer = json_decode($session->get('form_container_page_' . $value, '', 'form-design-' . $dataFormId));
								}
												...
												foreach ($fields as $index => $item)
												{
														...
														if (!empty($pageTemplate->dataField))
														{
																foreach ($pageTemplate->dataField as $pageTemp)
																{
																		...
																		{
																				$getSettings = json_decode($pageTemp->field_settings);
																				$getSettings->id = $tableField->field_id;
																				$fieldSettings = json_encode($getSettings);
																		}
																}
														}
														...
														$parsedFields[] = json_decode($fieldSettings);
