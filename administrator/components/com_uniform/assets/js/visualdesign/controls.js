        ...
        return function (JSNVisualDesign, language) {
            ...
            JSNVisualDesign.register('name', {
                ...
                params:{
                    ...
                    values:{
                        extra:{
                            type:'group',
                            decorator:'<div class="row-fluid"><div class="span6 jsn-items-list-container" id="jsn-field-name"><label for="option-name-itemlist" class="control-label">Fields</label><ul class="jsn-items-list ui-sortable"><vtitle/><vfirst/><vmiddle/><vlast/></ul></div><div id="jsn-name-default-titles" class="span6"><items/></div></div>',
            ...
            JSNVisualDesign.register('address', {
                ...
                tmpl:'<div class="control-group {{if hideField}}jsn-hidden-field{{/if}} jsn-group-field">' +
                    ...
                    '{{if vcity}}<div class="span6"><input type="text" class="jsn-input-xxlarge-fluid" placeholder="' + language['CITY'] + '" /></div>{{/if}}' +
                    '{{if vstate}}<div class="span6"><input type="text" class="jsn-input-xxlarge-fluid" placeholder="' + language['STATE_PROVINCE_REGION'] + '" /></div>{{/if}}' +
                    '</div>{{/if}} {{if vcode || vcountry}}<div class="row-fluid">' +
                    '{{if vcode}}<div class="span6"><input type="text" class="jsn-input-xxlarge-fluid" placeholder="' + language['POSTAL_ZIP_CODE'] + '" /></div>{{/if}}' +
                    '{{if vcountry}}<div class="span6"><select class="jsn-input-xxlarge-fluid">{{each(i, val) country}}<option value="${val.text}" {{if val.checked == true || val.checked=="true"}}selected{{/if}}>${val.text}</option>{{/each}}</select></div>{{/if}}' +
                    '</div>{{/if}}</div></div>'
            });
            //Website controls
