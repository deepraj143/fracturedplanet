...
}if(joms.jQuery("#appid").length!=0){e=joms.jQuery("#appid").val();}var c=f.data("filter");var a=f.data("filterid");jax.call("community","activities,ajaxGetOlderActivities",b,c,a);
},prependOldStream:function(b){container=joms.jQuery(".cMain > #activity-stream-container");var a=joms.jQuery(b).find("li");container.find("li[data-streamid]").last().after(a);
container.find("#activity-more .more-activity-text").show();container.find("#activity-more .loading").hide();},loadNewStream:function(){var a=joms.jQuery("#activity-stream-container ul.cStreamList");
jax.loadingFunction=function(){};jax.call("community","activities,ajaxGetRecentActivitiesCount",a.find("li[data-streamid]").first().data("streamid"),a.data("filter"),a.data("filterid"));
},announceNewStream:function(c,a,b){if(c>0){joms.jQuery(".joms-latest-activities-container").html('<div class="cAlert alert-success"><a href="#showNewStream" onclick="joms.activities.showNewStream();">'+b+"</div>").show();
...
cWindowShow(c,d,450,100);},unpublish:function(a){jax.call("community","groups,ajaxUnpublishGroup",a);},leave:function(a){var b='jax.call("community", "groups,ajaxShowLeaveGroup", "'+a+'");';
...
cWindowShow(a,"",450,100);},ajaxUpload:function(c,d,b){var a=joms.jQuery("#jsform-uploadavatar").prop("action");joms.jQuery.ajaxFileUpload({url:a,secureuri:false,fileElementId:"filedata",dataType:"json",beforeSend:function(){},complete:function(){},success:function(l,f){if(l.error=="true"){joms.jQuery("span.error").remove();
joms.jQuery("#avatar-upload").prepend('<span class="error">'+l.msg+"</span>");return false;}else{a=l.msg;joms.jQuery("span.error").remove();var h="";var j="";
joms.jQuery("#thumb-crop").css("min-height",0);joms.jQuery("#large-avatar-pic").prop("src",a);joms.jQuery("#thumb-hold  >img").prop("src",a);joms.jQuery("div.status-anchor img").prop("src",a);
joms.jQuery("#large-avatar-pic").load(function(){var n=Math.round((64/160)*joms.jQuery("#large-avatar-pic").width());var o=Math.round((64/160)*joms.jQuery("#large-avatar-pic").height());
joms.jQuery("#thumb-hold  >img").css({width:n+"px",height:o+"px",marginLeft:"0px",marginTop:"0px"});joms.photos.ajaxImgSelect();});switch(c){case"event":h=joms.jQuery(".js-focus-avatar img").prop("src");
var e=a.replace("event-","");joms.jQuery(".js-focus-avatar img").prop("src",e);if(h.search("assets/event.png")>-1){if(b){selected=joms.jQuery("input:radio[name=repeattype]:checked").val();
...
}else{jax.call("community","photos,ajaxUploadAvatar",c,d);}}break;case"group":h=joms.jQuery(".js-focus-avatar img").prop("src");var k=a.replace("group-","");
joms.jQuery(".js-focus-avatar img").prop("src",k);if(h.search("assets/group.png")>-1){jax.call("community","photos,ajaxUploadAvatar",c,d);}break;case"profile":h=joms.jQuery(".js-focus-avatar img").prop("src");
if(l.info.length!=0){var m=l.info;var g=m.replace("thumb_","");joms.jQuery(".status-author > img").prop("src",m);joms.jQuery('.cStream-Avatar img[data-author="'+d+'"]').prop("src",m);
joms.jQuery(".js-focus-avatar img").prop("src",g);}if(h.search("assets/user-")>-1){jax.call("community","photos,ajaxUploadAvatar",c,d);}break;}}}});return false;
},ajaxImgSelect:function(){var a=document.getElementById("large-avatar-pic");var c=joms.jQuery(a).height();var b=160;if(c<160){b=c;}if(c>160){c=160;}joms.jQuery("#large-avatar-pic").imgAreaSelect({maxWidth:160,maxHeight:160,handles:true,aspectRatio:"1:1",x1:0,y1:0,x2:b,y2:c,show:false,hide:true,enable:false,parent:"#cWindowContentWrap",minHeight:64,minWidth:64,onInit:joms.photos.previewThumb,onSelectChange:joms.photos.previewThumb});
...
}});},newLike:function(d){var b=joms.jQuery(d).parent();var c=this.extractData(b.attr("id"));joms.jQuery(d).attr("onclick","");var a=joms.jQuery(joms.jQuery(d).children()[1]).html();
...
joms.jQuery(joms.jQuery(d).children()[1]).html(a);}});},newDislike:function(d){var b=joms.jQuery(d).parent();var c=this.extractData(b.attr("id"));joms.jQuery(d).attr("onclick","");
...
var b=this.extractData(a.attr("id"));joms.jQuery(c).attr("onclick","");joms.ajax.call("system,ajaxDislike",[b.element,b.itemid],{success:function(d){a.replaceWith(d);
...
joms.jQuery("input:file").parent().css("top","230px");joms.jQuery("#cwin_close_btn").click(function(){if(a){window.location.reload();}});},viewFile:function(a,c){var b='jax.call("community", "files,ajaxviewFiles","'+a+'","'+c+'")';
...
}},cover:{type:null,parentId:null,init:function(){joms.jQuery(".js-navbar-options, .js-navbar-icon").dropdown({toggle:false});var b=joms.jQuery(".js-bar-collapse");
...
},updateCover:function(a){cWindowHide();joms.jQuery("#"+joms.cover.parentId).prop("src",a);},uploader:function(a){joms.jQuery.ajaxFileUpload({url:a,secureuri:false,fileElementId:"uploadCover",dataType:"json",beforeSend:function(){},complete:function(){},success:function(c,b){if(c.success==true){joms.cover.updateCover(c.path);
...
}}});}}});joms.jQuery(document).click(function(){joms.toolbar.close();});function update_counter(a,c){if(!c){c=0;}var b=parseInt(joms.jQuery(a).html(),10);
c=parseInt(c,10);if(b<=1){joms.jQuery(a).css("display","none");}else{joms.jQuery(a).html(b+c);}}function get_cookies_array(){var d={};if(document.cookie&&document.cookie!=""){var b=document.cookie.split(";");
